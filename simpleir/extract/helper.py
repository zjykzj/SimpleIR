...
class ExtractHelper:
    ...
    def _save_part_feat(self, save_feat_list, part_file_path):
    ...
    def run(self, dst_root, save_interval: int = 5000):
        ...
        for batch in tqdm(self.dataloader):
            ...
            # save_feat_list.extend(np.concatenate([paths, outputs], axis=0))
            if len(save_feat_list) > save_interval:
                self._save_part_feat(save_feat_list, os.path.join(dst_root, f'part_{part_count}.csv'))
                part_count += 1
                del save_feat_list
                save_feat_list = list()
